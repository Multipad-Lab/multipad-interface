import ComparisonStyleWrapper from "./Comparison.style"

const Comparison = () => {
  return (
    <ComparisonStyleWrapper id='comparison'>
      <h4 className='widget_title'>Comparison</h4>
      <div className='comparison'>
        <p>
          It has roots in a piece of classical Latin literature from 45 BC, making it over 2000 years old. Richard
          McClintock, a Latin professor at Hampden-Sydney College in Virginia, looked up one of the more obscure Latin
          words, consectetur cites of the word in classical literature. Standard chunkde Finibus Bonorum et Malorum" by
          Cicero are also reproduced in their exact original making it over 2000 years old. Richard McClintock
        </p>
      </div>
    </ComparisonStyleWrapper>
  )
}

export default Comparison
