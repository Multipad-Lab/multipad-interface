...
import { useForm } from "src/hooks/admin-project-details/useForm"
import { chainList } from "src/const/admin-project-details"
import TextEditor from "./TextEditor"
import Spin from "src/components/Spin/Spin"
import { useSubmit } from "src/hooks/admin-project-details/useSubmit"
...
const ApplyForm = () => {
  const { formInit, form, handleChangeForm, loadingInit, handleRefetchData } = useForm()
  const { handleSubmit, loadingSubmit } = useSubmit({ form, handleRefetchData })
  const { id } = useParams()
      ...
      <Spin spinning={loadingSubmit || loadingInit}>
        <div className='project-form'>
          <div className='form_widgets'>
            ...
            {/* <div className='form_widgets form_check' style={{ marginTop: 24, marginBottom: 12 }}>
              <h5>PROJECT RULE</h5>

              <label>
                Riskless
                <input
                  onChange={(e) => handleChangeForm({ field: "label", value: [e.target.value] })}
                  type='radio'
                  name='label'
                  value={"riskless"}
                  checked={form.label[0] === "riskless"}
                />
              </label>
              <label>
                Shielded
            ...
            {paymentList.map((item) => {
              return (
                <label key={item.symbol}>
                  {item.label}
                  <input
                    onChange={(e) => handleChangeForm({ field: "paymentSymbol", value: e.target.value })}
                    type='radio'
                    name='paymentSymbol'
                    value={item.symbol}
                    checked={form.paymentSymbol === item.symbol}
                  />
                </label>
              )
            })}
          ...
          <div className='form_widgets'>
            <div className='form-group'>
              {formInit?.status === 0 && (
                <Button
                  style={{ color: "#ffffff", height: 60, marginBottom: 24 }}
                  variant='outline'
                  href={`${routeTo(routesName.ADMIN_PROJECT_DETAILS_LAUNCHPAD, { id })}`}
                >
